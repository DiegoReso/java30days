...
- Desafio do `sacar()` na classe Conta: validar valor inválido **e** saldo insuficiente **antes** de mexer no saldo; lançar exceção com mensagem clara; testar o caminho feliz e o de erro no `main` (com `try/catch`), confirmando que o saldo não muda quando a regra bloqueia.